          ...
          <ComponentContainer className='flex lg:flex-row flex-col'>
            <FirstColumn className='flex flex-col lg:w-1/2 w-auto justify-between'>
              ...
              <DetailsBox className='flex flex-col lg:gap-2 gap-1 pb-4'>
                {data.Contact.Details.map((detail: IconsProps) => (
                  <Detail as='a' href='#' key={detail.id} className='flex flex-row items-center gap-4'>
                    {detail?.FAIcon?.Icon &&
                      <FAIcon
                        icon={detail?.FAIcon?.Icon}
                        width={Number(detail.FAIcon.Width)}
                        height={Number(detail.FAIcon?.Width)}
                      />
                    }
                    <span>
                      {detail.Title}
              ...
              <SocialsContainer className='flex flex-wrap gap-x-14 gap-y-6'>
                {data.Contact.Social.map((icon: IconsProps) => (
                  <StyledSocial as='a' href='#' key={icon.id} className='flex flex-row items-center gap-4 w-fit'>
                    {icon?.FAIcon?.Icon &&
                      <FAIcon
              ...
              {isMobile
                ...
                <InterestsList>
                  <Swiper
                    ...
                    className='w-full'
                  >
                    {data.Contact.Interests.map((interest: IconsProps, index: number) => (
